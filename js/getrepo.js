$(function () {
    ...
    $.get('https://api.github.com/users/EsriJapan/repos').then(function (repos) {
        ...
        for (var i = 0; i < repos.length; i++) {
        		...
        		for(var k = 0;  k < data_filter.length; k++) {
        			if(data_filter[k] === name_split[j]) {
        				sametag = true;
							...
							else if(name_split[j] === "esrijapan.github.io") {
								sametag = true;
							}
        		}
        		// 同名のタグ候補が無い場合
        	...
        	// リポジトリの言語
        	if(repos[i].language == 'C#' || repos[i].language == 'Objective-C' || repos[i].language == 'Java' || repos[i].language == 'JavaScript' || repos[i].language == 'HTML') {
        		console.log(repos[i]);
        		var repo_class;
	        	var header_class;
	        	var h3_class;
